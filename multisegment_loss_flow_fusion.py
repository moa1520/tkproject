...
class MultiSegmentLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        loc_data, conf_data, trans_segments, trans_logits, center_data = predictions
        num_batch = loc_data.size(0)

        num_priors = loc_data.size(1)
        # coarse_num_priors = loc_data.size(1)
        # # coarse prediction 갯수랑 refine prediction 갯수 해결하기
        # refine_num_priors = trans_logits.size(1)

        num_classes = self.num_classes
        priors = torch.Tensor(
            [(c + 0.5) / num_priors for c in range(num_priors)]).unsqueeze(-1).cuda()
        ...
        with torch.no_grad():
            for idx in range(num_batch):
                ...
                match gt
                '''
                K = loc_data.size(1)    # Number of predicted action instances
                # Number of ground truth's action instances
                N = truths.size(0)
