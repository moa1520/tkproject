        ...
        for offset in offsetlist:
            ...
            if save_offset:
                ...
                for anno in cur_annos:
                    ...
                    start_s = np.clip(int(round(s - d / 2)),
                                      0, clip_length - 1)
                    start_e = np.clip(int(round(s + d / 2)),
                                      0, clip_length - 1) + 1
                    start[start_s: start_e] = id
                    end_s = np.clip(int(round(e - d / 2)), 0, clip_length - 1)
                    end_e = np.clip(int(round(e + d / 2)),
                                    0, clip_length - 1) + 1
                    end[end_s: end_e] = id
...
class ANET_Dataset(Dataset):
    def __init__(self,
        ...
        self.training_list, self.th = split_videos(
            video_info, clip_length, stride, binary_class)
