        ...
        for offset in offsetlist:
            ...
            if save_offset:
                ...
                for anno in cur_annos:
                    s, e, id = anno
                    d = max((e - s) / 10.0, 2.0)
                    start_s = np.clip(int(round(s - d / 2.0)),
                                      0, clip_length - 1)
                    start_e = np.clip(int(round(s + d / 2.0)),
                                      0, clip_length - 1) + 1
                    start[start_s: start_e] = 1
                    end_s = np.clip(int(round(e - d / 2.0)),
                                    0, clip_length - 1)
                    end_e = np.clip(int(round(e + d / 2.0)),
                                    0, clip_length - 1) + 1
                    end[end_s: end_e] = 1
...
class THUMOS_Dataset(Dataset):
    ...
    def augment_(self, input, annos, th):
        ...
        try:
            gt = random.choice(
                list(filter(lambda x: x[1] - x[0] > 2 * th, annos)))
    ...
    def __getitem__(self, idx):
        ...
        flow_data = np.load(os.path.join(
            config['dataset']['training']['flow_data_path'], sample_info['video_name'] + '.npy'))
        ...
        flow_input_data = flow_data[:, offset: offset + self.clip_length]
        c, t, h, w = input_data.shape
        f_c, f_t, f_h, f_w = flow_input_data.shape
        if t < self.clip_length:
            ...
            f_pad_t = self.clip_length - f_t
            f_zero_clip = np.zeros(
                [f_c, f_pad_t, f_h, f_w], flow_input_data.dtype)
            flow_input_data = np.concatenate([flow_input_data, f_zero_clip], 1)

        # random crop and flip
        if self.training:
            input_data = self.random_flip(self.random_crop(input_data))
            flow_input_data = self.random_flip(
                self.random_crop(flow_input_data))
        else:
            input_data = self.center_crop(input_data)
            flow_input_data = self.center_crop(flow_input_data)

        # import pdb;pdb.set_trace()
        input_data = torch.from_numpy(input_data).float()
        flow_input_data = torch.from_numpy(flow_input_data).float()
        if self.rgb_norm:
            input_data = (input_data / 255.0) * 2.0 - 1.0
            flow_input_data = (flow_input_data / 255.0) * 2.0 - 1.0
        ...
        return input_data, flow_input_data, target, scores
