...
from common.anet_dataset import ANET_Dataset, detection_collate

from common.configs import config
from multisegment_loss import MultiSegmentLoss
from networks.network_testing_ssl_anet import PTN
...
def one_forward(net, clips, targets, scores=None, training=True, ssl=True):
    clips = clips.cuda()
    targets = [t.cuda() for t in targets]

    if training:
        if ssl:
            output = net(clips, proposals=targets, ssl=ssl)
        else:
            output = net(clips, ssl=False)
    else:
        with torch.no_grad():
            output = net(clips)
    if ssl:
        ...
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            for i in range(len(flags)):
                if flags[i] and config['training']['ssl'] > 0:
                    loss_trip += one_forward(net, ssl_clips[i].unsqueeze(
                        0), [ssl_targets[i]], training=training, ssl=True) * config['training']['ssl']
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': net.feature_pyramid_net.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.poisition_embedding.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.transformer.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.loc_mixup_branch.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.conf_mixup_branch.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.boundary_max_pooling.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.center_head.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.prop_loc_head.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
        {'params': net.prop_conf_head.parameters(),
         'lr': learning_rate,
         'weight_decay': weight_decay},
    ])
    """
    ...
    for i in range(start_epoch, max_epoch + 1):
        run_one_epoch(i, net=net, optimizer=optimizer, data_loader=train_data_loader,
