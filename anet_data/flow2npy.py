...
def sub_processor(pid, video_list):
    for video_name in video_list:
        tmp = []
        print(video_name)
        flow_x_files = sorted(glob.glob(os.path.join(
            flow_frame_path, video_name, 'flow_x_*.jpg')))
        flow_y_files = sorted(glob.glob(os.path.join(
            flow_frame_path, video_name, 'flow_y_*.jpg')))
...
for i in range(thread_num):
    if i == thread_num - 1:
        ...
        sub_files = video_list[i *
                               per_process_video_num: (i + 1) * per_process_video_num]
