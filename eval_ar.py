...
def run_evaluation(ground_truth_filename, proposal_filename,
                   max_avg_nr_proposals=100, tiou_thresholds=np.linspace(0.5, 0.95, 10),
                   subset='validation'):
...
def plot_metric(average_nr_proposals, average_recall, recall, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    colors = ['k', 'r', 'yellow', 'b', 'c',
              'm', 'b', 'pink', 'lawngreen', 'indigo']
    ...
    ax.legend([handles[-1]] + handles[:-1],
              [labels[-1]] + labels[:-1], loc='best')
