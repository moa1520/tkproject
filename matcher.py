...
from common import segment_utils
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_segment = outputs["pred_segments"].flatten(
            0, 1)  # [batch_size * num_queries, 4]

        scale_factor = torch.stack([t["length"] for t in targets], dim=0)
        out_segment_scaled = out_segment * \
            scale_factor.unsqueeze(1).repeat(
                1, num_queries, 1).flatten(0, 1).repeat(1, 2)

        tgt_ids = torch.cat([v["labels"] for v in targets])
        tgt_segment = torch.cat([v["segments"] for v in targets])
        tgt_segment_scaled = torch.cat(
            [v["segments"] * v['length'] for v in targets])
        ...
        if tgt_segment.dim() > 1:
            ...
            cost_siou = - \
                segment_utils.generalized_segment_iou(
                    tgt_segment_scaled, out_segment_scaled)
        ...
        # Final cost matrix
        C = self.cost_segment * cost_segment + self.cost_class * \
            cost_class + self.cost_siou * cost_siou
