...
def segment_iou(target_segment, candidate_segments):
    ...
    """
    tt1 = np.maximum(target_segment[0], candidate_segments[:, 0].astype(float))
    tt2 = np.minimum(target_segment[1], candidate_segments[:, 1].astype(float))
    # Intersection including Non-negative overlap score.
    segments_intersection = (tt2 - tt1).clip(0)
    # Segment union.
    segments_union = (candidate_segments[:, 1].astype(float) - candidate_segments[:, 0].astype(float)) \
