...
from common.anet_dataset import get_video_info, load_json
...
def sub_processor(lock, pid, video_list):
    ...
    for video_name in video_list:
        ...
        for cl in range(1, num_classes):
            ...
            tmp, count = softnms_v2(
                tmp, sigma=nms_sigma, top_k=top_k, score_threshold=1e-9)
            res[cl, :count] = tmp
            sum_count += count
...
for i in range(thread_num):
    if i == thread_num - 1:
        ...
        sub_video_list = video_list[i *
                                    per_thread_video_num: (i + 1) * per_thread_video_num]
...
output_dict = {"version": "ActivityNet-v1.3",
               "results": dict(result_dict), "external_data": {}}
