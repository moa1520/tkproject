...
from common.mha import MultiheadAttention
...
class TemporalEncoder(nn.Module):
    def __init__(self, input_dim, dropout, temporal_scale=100, window_size=9):
        ...
        dim_feedforward = 256
        self.self_atten = SelfAttention(
            input_dim, num_heads=8, dropout=0.1, temporal_scale=temporal_scale, window_size=window_size)
...
class SelfAttention(nn.Module):
    def __init__(self, input_dim, num_heads, dropout, temporal_scale, window_size=9):
        ...
        self.scale_attention = MultiheadAttention(input_dim,
                                                  ...
                                                  dropout=dropout)
        self.mask_matrix = nn.Parameter(self._mask_matrix(
            window_size).float(), requires_grad=False)
        ...
        for i in range(local_len):
            for j in range(self.temporal_scale):
                for k in range(w_len):
                    m[0, i, j, min(max(j - w_len // 2 + k, 0),
                                   self.temporal_scale - 1)] = False
