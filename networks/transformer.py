...
class Graph_Transformer(nn.Module):
    ...
    def forward(self, src, src_mask, query_embed, pos_embed):
        ...
        encoder_mask = None
        memory = self.encoder(src, mask=encoder_mask,
                              src_key_padding_mask=src_mask, pos=pos_embed)
        ctx = memory

        tgt_mask = None
        hs, edge = self.decoder(tgt, ctx.permute(1, 0, 2), tgt_mask=tgt_mask,
                                memory_key_padding_mask=src_mask, pos=pos_embed, query_pos=query_embed)
