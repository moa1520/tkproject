    ...
    def forward(self, x):
        if self.norm is not None and type(self.norm) != str:
            ...
            for i in range(x.size(0)):
                x_norm.append(self.compute_norm(self.compute_norm(
                    x[i, :, :], dim=1), dim=0).unsqueeze(0))
        ...
        batch_index = torch.arange(bs).to(
            x.device).repeat_interleave(batch_list)
        batch_index = batch_index.view(
            (-1,) + (1,) * (x.dim() - 1)).expand_as(x)
...
class GraphEncoderDecoderAttention(nn.Module):
    def __init__(self, nhid, nheads, dropout, norm_type='bn', alpha=0.1, decoder_attn='ctx'):
        ...
        self.nhid = nhid
        self.nheads = nheads
        self.graph_attentions = [GraphEncoderDecoderAttentionLayer(
            nhid, nhid, nhid//nheads, dropout=dropout, alpha=alpha, concat=True) for _ in range(nheads)]
        ...
        self.graph_multihead_attn = nn.MultiheadAttention(
            nhid, nheads, dropout=dropout)
        self.decoder_attention = decoder_attn

    def forward(self, x, ctx_with_pos, ctx, src, adj):
        x = F.dropout(x, self.dropout)
        ctx = F.dropout(ctx, self.dropout)
        x = x + torch.cat([att(x, ctx_with_pos, adj)
                          for att in self.graph_attentions], dim=2)
...
class GraphSelfAttention(nn.Module):
    def __init__(self, nhid, nheads, dropout, norm_type='bn', alpha=0.1):
        ...
        self.nhid = nhid
        self.nheads = nheads
        self.graph_attentions = [GraphAttentionLayer(
            nhid, nhid//nheads, dropout=dropout, alpha=alpha, concat=True) for _ in range(nheads)]
        for i, attention in enumerate(self.graph_attentions):
            self.add_module('attention_{}'.format(i), attention)
        self.linear = nn.Linear(nhid, nhid)
...
class GraphEncoderDecoderAttentionLayer(nn.Module):
    ...
    def __init__(self, in_src_features, in_tgt_features, out_features, dropout, alpha,  concat=True):
        ...
        self.Ws = nn.Parameter(torch.empty(
            size=(in_src_features, out_features)))
        self.Wt = nn.Parameter(torch.empty(
            size=(in_tgt_features, out_features)))
    ...
    def _prepare_attentional_mechanism_input(self, Ws_ctx, Wt_h):
        ...
        all_combinations_matrix = torch.cat(
            [Ws_ctx_repeated_in_chunks, Wt_h_repeated_alternating], dim=2)
